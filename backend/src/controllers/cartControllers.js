...
const getAllCartedProducts=async (req,res,next)=>{
    try{
        const result=await pool.query("select c.cart_id,p.product_id,p.product_name,p.product_image,p.price,p.stock,p.is_available\
    ...
    try{
        ...
        for(const cartedProduct of cartedProducts){
            try{
                ...
                await con.query('delete from carts where user_id=$1',[req.user.user_id])
